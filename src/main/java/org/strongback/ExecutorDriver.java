            ...
            while (running) {
                ...
                for (int i = 0; i != numHighPriorityItems; ++i) {
                    ...
                    if (!running)
                        return;
                    try {
                        long start = timeSystem.currentTimeInMillis();
                        executable.execute(start);
                        long end = timeSystem.currentTimeInMillis();
                        if (end - start > longCycleTime) {
                            System.err.printf("Executing %s took %d ms\n",
                                    executable.getName(), end - start);
                        }
                    } catch (Throwable e) {
                        logger.error(e);
                    }
