...
public class HardwareSparkMAX implements Motor {
    ...
    @Override
    public void set(ControlMode mode, double value, double arbFeedFwd) {
        // spark.set(value);
        if (mode == Motor.ControlMode.DutyCycle) {
            value += arbFeedFwd;
        }
        getPID().setReference(value, mode.revControlType, slotID);
        setpoint = value;
    ...
    public Motor setScale(double ticksPerTurn, double gearRatio, double unitsPerRev) {
        if (ticksPerTurn * gearRatio * unitsPerRev == 0) {
        ...
        encoder.setVelocityConversionFactor(unitsPerRev / gearRatio / 60);
        // Encoder getPosition() returns rotations by default.
        encoder.setPositionConversionFactor(unitsPerRev / gearRatio);
        return this;
    }
